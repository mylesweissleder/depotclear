...
// San Diego Metro cities to scrape
const sdCities = [
  // Core San Diego
  'San Diego', 'La Jolla', 'Pacific Beach', 'Mission Beach', 'Ocean Beach',
  'Downtown San Diego', 'Hillcrest', 'North Park', 'South Park', 'Normal Heights',
  // North County Coastal
  'Carlsbad', 'Encinitas', 'Oceanside', 'Del Mar', 'Solana Beach',
  // North County Inland
  'Escondido', 'San Marcos', 'Vista', 'Rancho Bernardo', 'Poway',
  // East County
...
class DogDaycareScraper {
  ...
  async initialize() {
    console.log('🚀 Initializing browser for San Diego Metro...');
...
async function saveToDatabase(businesses) {
  let savedCount = 0;
  try {
    console.log('\n💾 Saving San Diego daycares to database...');
...
async function main() {
  ...
  try {
    ...
    console.log(`🏖️ Starting San Diego Metro scraper (${sdCities.length} cities)...\n`);

    for (const city of sdCities) {
      const businesses = await scraper.scrapeCity(city);
      allBusinesses.push(...businesses);
      await new Promise(resolve => setTimeout(resolve, 3000 + Math.random() * 2000));
    }

    console.log(`\n📊 Scraping Summary: ${allBusinesses.length} total daycares found in San Diego Metro`);
